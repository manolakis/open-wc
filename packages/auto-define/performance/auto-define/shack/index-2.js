...
import { autoDefine } from '../../../index.js';
...
const execute = async () => {
  const [shackApp, shackCart, shackItem] = autoDefine(ShackApp, ShackCart, ShackItem);

  const app = document.createElement(shackApp.get());
  app.page = 'mens_tshirts';
  app.categories = categories;
  document.body.appendChild(app);

  const $shackApp = await findElement(document.body, shackApp.get());
  const $shackItems = await findElements($shackApp.shadowRoot, shackItem.get());
  const $shackCart = await findElement($shackApp.shadowRoot, shackCart.get());
